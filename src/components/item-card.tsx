...
export function ItemCard({ item }: ItemCardProps) {
  return <Link className="card" href={`/catalog/${item.id}`}>
    ...
    <p className="subtle" style={{minHeight:42}}>{item.description || "Todavía no tiene descripción."}</p>
    ...
    <div className="row" style={{marginTop:16,fontSize:12,color:"var(--muted)"}}><span>{item.type?.name ?? "Sin clasificar"}</span><span>{item.updatedAt.toLocaleDateString("es")}</span></div>
  </Link>;
}
