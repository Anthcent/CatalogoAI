...
export function ItemTabs({ id, active }: { id: string; active: "canvas" | "properties" | "relations" | "history" }) {
  const tabs = [["canvas", `/catalog/${id}`, "Lienzo"], ["properties", `/catalog/${id}/properties`, "Propiedades"], ["relations", `/catalog/${id}/relations`, "Relaciones"], ["history", `/catalog/${id}/history`, "Historial"]] as const;
