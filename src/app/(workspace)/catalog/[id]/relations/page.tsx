...
const relationTypes = ["related to", "uses", "depends on", "derived from", "replaces", "complements", "generated from", "belongs to"];
...
export default async function RelationsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!item) notFound();
  return <><ItemTabs id={id} active="relations"/><main className="page" style={{maxWidth:900}}><span className="eyebrow">{item.publicCode}</span><h1>Relations</h1><form action={createRelationAction} className="card form-stack"><input type="hidden" name="sourceItemId" value={id}/><div className="relation-form"><select className="field" name="relationType">{relationTypes.map(type=><option key={type}>{type}</option>)}</select><select className="field" name="targetItemId" required><option value="">Choose an item</option>{candidates.map(candidate=><option value={candidate.id} key={candidate.id}>{candidate.publicCode} - {candidate.title}</option>)}</select><button className="btn primary">Create relation</button></div></form><div className="section-title"><h2>Outgoing</h2></div>{item.outgoing.length?item.outgoing.map(relation=><div className="card relation-row" key={relation.id}><span className="pill">{relation.relationType}</span><a href={`/catalog/${relation.target.id}`}><b>{relation.target.title}</b><small className="code">{relation.target.publicCode}</small></a><form action={deleteRelationAction}><input type="hidden" name="id" value={relation.id}/><input type="hidden" name="sourceItemId" value={id}/><button className="icon-btn" aria-label="Delete relation">Remove</button></form></div>):<p className="subtle">No outgoing relations yet.</p>}<div className="section-title"><h2>Referenced by</h2></div>{item.incoming.map(relation=><div className="card relation-row" key={relation.id}><span className="pill">{relation.relationType}</span><a href={`/catalog/${relation.source.id}`}><b>{relation.source.title}</b><small className="code">{relation.source.publicCode}</small></a></div>)}</main></>;
}
