...
import { embedTexts } from "@/features/ai/service";
...
export async function indexItem(itemId: string) {
  ...
  await db.$transaction(async (transaction) => {
    ...
    for (const [index, chunk] of chunks.entries()) {
      ...
      await transaction.embedding.create({
        data: {
          itemId,
...
export async function semanticMatches(query: string) {
  ...
  const embeddings = await db.embedding.findMany({
    select: { itemId: true, textChunk: true, vector: true },
    take: 2000,
  });
  const best = new Map<
    string,
    { itemId: string; similarity: number; textChunk: string }
  >();
  for (const embedding of embeddings) {
    if (!Array.isArray(embedding.vector)) continue;
    const similarity = cosineSimilarity(vector, embedding.vector.map(Number));
    if (
      similarity >= 0.45 &&
      similarity > (best.get(embedding.itemId)?.similarity ?? -1)
    )
      best.set(embedding.itemId, {
        itemId: embedding.itemId,
        similarity,
        textChunk: embedding.textChunk,
      });
  }
  return [...best.values()]
